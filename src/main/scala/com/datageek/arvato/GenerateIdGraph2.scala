...
object GenerateIdGraph2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Define the output directory path & clean it
    val outputDir = "file:///Users/Kalid/IdeaProjects/etl/bdr-main/target/output/"
    ...
    /** For a given source Vertex,
      ...
      */
    def generateMergeResult(sourceId: Long): Unit = {
      /** ********** step 2.B **********
      ...
      ).sortBy(vertex => (vertex._1, vertex._3), ascending = false).repartition(1)
      ...
      /** add index to each column name */
      def addIdx(iter: Iterator[(String, String, Double)]): Iterator[(String, String, Double)] = {
        var res = List[(String, String, Double)]()
        var idType = ""
        var idx = 1
        for (elmt <- iter){
          if (idType == elmt._1) idx += 1 else {
            idType = elmt._1
            idx = 1
          }
          res.::=(elmt._1.toLowerCase() + idx.toString(),elmt._2, elmt._3)
        }
        res.iterator
      }

      val finalRDD: RDD[(String, String, Double)] = cntedVerticesFinalInfo.mapPartitions(addIdx)

      //println(finalRDD.collect.mkString("\n"))

      val rowRDD: RDD[Map[String, Any]] = finalRDD.map(data =>
        Map(data._1 -> data._2, data._1 + "_wt" -> data._3, sourceType -> sourceIdValue))
      val partitions: RDD[String] = rowRDD.mapPartitions(ite => {
        val json = new JSONObject()
        for (elem <- ite) {
          for (elem <- elem) {
            json.put(elem._1, elem._2)
          }
        }
        List(json.toString).iterator
      })

      // TODO: insert the result cntedVerticesFinalInfo into table T_merge (Hive)
      sqlContext.read.json(partitions)
        .write.format("orc").mode(saveMode = SaveMode.Append).saveAsTable("arvato_temp.t_merge")
    }
    ...
    for (vid <- sourceIDList) generateMergeResult(vid)
  ...
  def orderToWgt(order: Double, maxOrder: Double = 10, isAscending: Boolean = true): Double = {
    ...
    else (1 + math.log(1 + 1 / (maxOrder - order + 1)) / math.log(2)) / 2.0
  }
